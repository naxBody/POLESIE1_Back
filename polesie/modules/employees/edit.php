...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    $formData = [
        ...
        "is_active" => isset($_POST["status"]) && $_POST["status"] !== "terminated" ? 1 : 0
    ];
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("UPDATE employees SET full_name=:fn,position=:pos,department=:dep,email=:em,phone=:ph,hire_date=:hd,salary=:sal,is_active=:ia,updated_at=NOW() WHERE id=:id");
            $stmt->execute([":fn"=>$formData["full_name"],":pos"=>$formData["position"],":dep"=>$formData["department"],":em"=>$formData["email"]?:null,":ph"=>$formData["phone"]?:null,":hd"=>$formData["hire_date"],":sal"=>$formData["salary"]?:null,":ia"=>$formData["is_active"],":id"=>$id]);
            $success = true;
