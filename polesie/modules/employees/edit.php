...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("UPDATE employees SET full_name=:fn,position=:pos,department=:dep,email=:em,phone=:ph,hire_date=:hd,salary=:sal,is_active=:ia,notes=:nt,updated_at=NOW() WHERE id=:id");
            $stmt->execute([":fn"=>$formData["full_name"],":pos"=>$formData["position"],":dep"=>$formData["department"],":em"=>$formData["email"]?:null,":ph"=>$formData["phone"]?:null,":hd"=>$formData["hire_date"],":sal"=>$formData["salary"]?:null,":ia"=>$formData["is_active"],":nt"=>$formData["notes"]?:null,":id"=>$id]);
            $success = true;
